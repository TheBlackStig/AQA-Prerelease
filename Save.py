...
#Screenshot
def SaveGame(PlayerName, PlayerTiles, PlayerTilesPlayed, PlayerScore, fileName):
    #Clear the file
    with open(fileName+".txt", "w") as file:
        pass
    file.close()
    with open(fileName+".txt", "a") as file:
        file.write(PlayerName)              #String
        file.write(PlayerTiles)             #String
        file.write(PlayerTilesPlayed)       #Int
        file.write(PlayerScore)             #Int
        file.write("\n")
    file.close()
...
def LoadGame(fileName):
    ...
    #Read player 1 
    p1Name = file_contents[0]
    p1Tiles = file_contents[1]
    p1TilesPlayed = file_contents[2]
    p1Score = file_contents[3]
    
    #Read player 2
    p2Name = file_contents[5]
    p2Tiles = file_contents[6]
    p2TilesPlayed = file_contents[7]
    p2Score = file_contents[8]
        
    return p1Name, p1Tiles, int(p1TilesPlayed), int(p1Score), p2Name, p2Tiles, int(p2tilesPlayed), int(p2Score)
...
def PlayGame(AllowedWords, TileDictionary, RandomStart, StartHandSize, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles):
    ...
    if RandomStart:
        ...
        #Screenshot
        if input("Would you like to load a game? (Y/N)").upper() == "Y":
            PlayerOneName, PlayerOneTiles, PlayerOneTilesPlayed, PlayerOne Score, PlayerTwoName, PlayerTwoTiles, PlayerTwoTilesPlayed, PlayerTwoScore = LoadGame(file_name)
    else:
    ...
    while PlayerOneTilesPlayed <= MaxTilesPlayed and PlayerTwoTilesPlayed <= MaxTilesPlayed and len(PlayerOneTiles) < MaxHandSize and len(PlayerTwoTiles) < MaxHandSize:
        ...
        PlayerTwoTiles, PlayerTwoTilesPlayed, PlayerTwoScore, TileQueue = HaveTurn(
            PlayerTwoName, PlayerTwoTiles, PlayerTwoTilesPlayed, PlayerTwoScore, TileDictionary, TileQueue, AllowedWords, MaxHandSize, NoOfEndOfTurnTiles)
        #Screenshot
        if input("Do you want to save? (Y/N)").upper() == "Y":
            fileName = input("Enter file name: ")
            SaveGame("Player One", PlayerOneTiles, PlayerOneTilesPlayed, PlayerOneScore, fileName)
            SaveGame("Player Two", PlayerTwoTiles, PlayerOneTilesPlayed, PlayerOneScore, fileName)        
